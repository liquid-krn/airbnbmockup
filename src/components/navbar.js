 ...
 <div className="flex items-center space-x-4 ">
  {icons.slice(0, 2).map((icon, index) => (
    <button
      ...
      className="ml-4 text-gray-500 hover:text-yellow-300 transition duration-300 text-2xl"
    >
      {icon}
    </button>
  ))}
...
<div className="flex items-center space-x-4">
  {icons.slice(2, 4).map((icon, index) => (
    <button
      ...
      className="mr-4 text-gray-500 hover:text-yellow-300 transition duration-300 text-xl sm:text-2xl"
    >
      {icon}
    </button>
  ))}
